...
public class SongSelectManager : MonoBehaviour, IStoreListener
{
    ...
    private void UpdateSong(int musicIndex)
    {
        ...
        reference.GetValueAsync().ContinueWith(task =>
        {
            if (task.IsCompleted)
            {
                ...
                {
                    //현재 사용자가 구매한 이력이 있는경우 곡을 플레이 할 수 있습니다.
                    ...
                    if (disabled)
                    {
                        ...
                        disableAlertUI.text = "플레이 할수 없는 곡입니다.";
                        startUI.gameObject.SetActive(false);
                    }
                }
            }
    ...
    {
        bool success = true;
        // 아래 소스코드는 안드로이드에서 실행했을때만 정상적으로 동작합니다.
