...
public class NoteController : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        string beatInormation = reader.ReadLine();
        bpm = Convert.ToInt32(beatInormation.Split(' ')[0]);
        divider = Convert.ToInt32(beatInormation.Split(' ')[1]);
        startingPoint = (float)Convert.ToDouble(beatInormation.Split(' ')[2]);
        //1초 마다 떨어지는 비트 개수
        beatCount = (float)bpm / divider;
        //비트가 떨어지는 간격시간
        beatInterval = 1 / beatCount;
        //각 비트들이 떨어지느 위치 및 시간 정보를 읽습니다.
        string line;
    ...
    void GameResult()
    {
        PlayerInformation.maxBombo = GameManager.instance.maxCombo;
