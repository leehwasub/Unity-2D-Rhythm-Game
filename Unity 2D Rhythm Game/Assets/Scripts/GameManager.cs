...
public class GameManager : MonoBehaviour
{
    ...
    private SpriteRenderer[] trailSpriteRenders;
    ...
    void Start()
    {
        ...
        trailSpriteRenders = new SpriteRenderer[trails.Length];
        for(int i = 0; i<trails.Length; i++)
        {
            trailSpriteRenders[i] = trails[i].GetComponent<SpriteRenderer>();
        }
    }
    ...
    void Update()
    {
            ...
            for(int i = 0; i < Input.touchCount; i++)
            {
                ...
                if(tempTouch.phase == TouchPhase.Began)
                {
                    ...
                    if(Physics.Raycast(ray, out hit, Mathf.Infinity))
                    {
                        ...
                        else if (hit.collider.name == "Trail 2")
                        {
                            ShineTrail(1);
                            objectPooler.Judge(2);
                        }
                        else if (hit.collider.name == "Trail 3")
                        {
                            ShineTrail(2);
                            objectPooler.Judge(3);
                        }
                        else if (hit.collider.name == "Trail 4")
                        {
                            ShineTrail(3);
        ...
        */
        ...
        for(int i=0; i<trailSpriteRenders.Length; i++)
        {
            Color color = trailSpriteRenders[i].color;
            color.a -= 0.01f;
            trailSpriteRenders[i].color = color;
        }
    }

    public void ShineTrail(int index)
    {
        Color color = trailSpriteRenders[index].color;
        color.a = 0.32f;
        trailSpriteRenders[index].color = color;
    }
    ...
    public void processJudge(judges judge, int noteType)
    {
        ...
        //MISS 판정을 받은 경우 콤보를 종료하고 점수를 깎습니다.
        ...
        // BAD 판정을 받은경우 콤보를 종료하고, 점수를 조금 깎습니다.
        ...
        //PERFECT혹은 GOOD 판정의 경우 콤보 및 점수를 올립니다.
        else
        {
